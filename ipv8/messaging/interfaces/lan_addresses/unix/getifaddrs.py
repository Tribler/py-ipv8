...
class GetIfAddrs(AddressProvider):
    ...
    def get_addresses(self) -> set[str]:
        ...
        try:
            next_addr = p_if_adresses
            while next_addr:
                p_address = next_addr.contents.ifa_addr
                netmask_is_zero = False  # Check for netmask "0.0.0.0"
                if next_addr.contents.ifa_netmask:
                    family = next_addr.contents.ifa_netmask.contents.sa_family
                    if family == 2:
                        socket_desc = cast(next_addr.contents.ifa_netmask, POINTER(sockaddr_in)).contents
                        netmask_is_zero = string_at(socket_desc.sin_addr, 4) == NETMASK0_V4
                    elif family == 10:
                        socket_desc6 = cast(next_addr.contents.ifa_netmask, POINTER(sockaddr_in6)).contents
                ...
                next_addr = next_addr.contents.ifa_next
        finally:
