...
if TYPE_CHECKING:
    ...
    from .requestcache import NumberCache, NumberCacheWithName, RequestCache
...
def retrieve_cache(cache_class: type[NumberCacheWithName]) -> Callable[[CTF], CTF]:
    """
    ...
    def decorator(func: CTF) -> CTF:
        ...
        def wrapper(self: Overlay, peer_or_addr: Address | Peer,
                    *payloads: Payload) -> Coroutine | None:
            try:
...
class EZPackOverlay(Overlay, ABC):
    ...
    def ez_send(self, peer: Peer, *payloads: Payload, sig: bool = True) -> None:
        ...
        """
        self._ez_senda(peer.address, *payloads, sig=sig)

    def _ez_senda(self, address: Address, *payloads: Payload, sig: bool = True) -> None:
        """
    ...
    def ezr_pack(self, msg_num: int, *payloads: Payload, sig: bool = True) -> bytes:
        """
    ...
    def _ez_unpack_auth(self,
                        ...
                        data: bytes) -> tuple[BinMemberAuthenticationPayload, GlobalTimeDistributionPayload, UT]:
        # UNPACK
        ...
        fmt: list[type[Serializable]] = [GlobalTimeDistributionPayload, payload_class]
        unpacked = self.serializer.unpack_serializable_list(fmt, remainder, offset=23)
        # ASSERT
    ...
    def _ez_unpack_noauth(self,
        ...
        # UNPACK
        fmt: list[type[Serializable]] = ([GlobalTimeDistributionPayload, payload_class] if global_time
                                         else [payload_class])
        unpacked = self.serializer.unpack_serializable_list(fmt, data, offset=23)
        # PRODUCE
...
# We can't put these at the top of the file: we need EZPackOverlay to be defined.
PayloadParams = ParamSpec("PayloadParams", bound=Payload)  # type: ignore[misc]
EZPackOverlayInst = TypeVar("EZPackOverlayInst", bound=EZPackOverlay)
LazyWrappable = Callable[Concatenate[EZPackOverlayInst, Peer, PayloadParams], Coroutine | None]
LazyWDWrappable = Callable[Concatenate[EZPackOverlayInst, Peer, PayloadParams], Coroutine | None]
LazyUnsignedWrappable = Callable[Concatenate[EZPackOverlayInst, Address, PayloadParams],
                                 Coroutine | None]
LazyUnsignedWDWrappable = Callable[Concatenate[EZPackOverlayInst, Address, PayloadParams],
                                   Coroutine | None]
MessageHandler = Callable[[EZPackOverlayInst, Address, bytes], None] | Callable[[EZPackOverlayInst, Address, bytes], Coroutine]

def lazy_wrapper(*payloads: type[Payload]) -> Callable[[LazyWrappable], MessageHandler]:
    """
    ...
    ::
        ...
        def on_message(peer: Peer,
            ...
            pass
    """
    def decorator(func: LazyWrappable) -> MessageHandler:
        @wraps(func)
        def wrapper(self: EZPackOverlayInst, source_address: Address, data: bytes) -> Coroutine | None:
            # UNPACK
            auth, _ = self.serializer.unpack_serializable(BinMemberAuthenticationPayload, data, offset=23)
            signature_valid, remainder = self._verify_signature(auth, data)
            ...
            return func(self, peer or Peer(auth.public_key_bin, source_address), *unpacked)
        return wrapper  # type: ignore[return-value]
    return decorator


def lazy_wrapper_wd(*payloads: type[Payload]) -> Callable[[LazyWDWrappable], MessageHandler]:
    """
    ...
    ::
        ...
        def on_message(peer: Peer,
            ...
            pass
    """
    def decorator(func: LazyWDWrappable) -> MessageHandler:
        @wraps(func)
        def wrapper(self: EZPackOverlayInst, source_address: Address, data: bytes) -> Coroutine | None:
            # UNPACK
            auth, _ = self.serializer.unpack_serializable(BinMemberAuthenticationPayload, data, offset=23)
            ...
            # ASSERT
            if not signature_valid:
                payloads_list = [payload_class.__name__ for payload_class in payloads]
                msg = f"Incoming packet {payloads_list!s} has an invalid signature"
                raise PacketDecodingError(msg)
            # PRODUCE
            ...
            if peer:
                peer.add_address(source_address)
            return func(self, peer or Peer(auth.public_key_bin, source_address), *output)
        return wrapper  # type: ignore[return-value]
    return decorator


def lazy_wrapper_unsigned(*payloads: type[Payload]) -> Callable[[LazyUnsignedWrappable], MessageHandler]:
    """
    ...
    ::
        ...
        def on_message(source_address: Address,
            ...
            pass
    """
    def decorator(func: LazyUnsignedWrappable) -> MessageHandler:
        @wraps(func)
        def wrapper(self: EZPackOverlayInst, source_address: Address, data: bytes) -> Coroutine | None:
            # UNPACK
...
def lazy_wrapper_unsigned_wd(*payloads: type[Payload]) -> Callable[[LazyUnsignedWDWrappable],
                                                                   MessageHandler]:
    """
    ...
    ::
        ...
        def on_message(source_address: Address,
            ...
            pass
    """
    def decorator(func: LazyUnsignedWDWrappable) -> MessageHandler:
        @wraps(func)
        def wrapper(self: EZPackOverlayInst, source_address: Address, data: bytes) -> Coroutine | None:
            @lazy_wrapper_unsigned(*payloads)
            def inner_wrapper(inner_self: EZPackOverlayInst, inner_source_address: Address,
                              *pyls: Payload) -> Coroutine | None:
