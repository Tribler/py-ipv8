...
from ipv8.community import Community, CommunitySettings  # noqa: I001
from ipv8.configuration import DISPERSY_BOOTSTRAPPER, get_default_configuration
from ipv8.messaging.interfaces.udp.endpoint import UDPv4Address, Address
...
with open("summary.txt", "w") as f:
    ...
    for key, values in RESULTS.items():
        ...
        f.write(f"\n{r_hostname} {r_address[0]}:{r_address[1]} {CONST_REQUESTS} {len(values)}")

with open("walk_rtts.txt", "w") as f:
    f.write("HOST_NAME ADDRESS RTT")
    for key, values in RESULTS.items():
        r_hostname, r_address = key
        f.writelines(f"\n{r_hostname} {r_address[0]}:{r_address[1]} {rtt}" for rtt in values)
