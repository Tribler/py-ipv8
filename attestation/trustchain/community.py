...
from .block import TrustChainBlock, ValidationResult, GENESIS_SEQ, UNKNOWN_SEQ
...
class TrustChainCommunity(Community):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.persistence = self.DB_CLASS(kwargs.get('working_directory', ''), self.DB_NAME)

        self.logger.debug("The trustchain community started with Public Key: %s",
                          self.my_peer.public_key.encode("hex"))
    ...
    def send_block(self, peer, block):
        ...
        payload = HalfBlockPayload.from_block(block)
        dist = GlobalTimeDistributionPayload(global_time).to_pack_list()

        packet = self._ez_pack(self._prefix, 1, [dist, payload], False)
        self.endpoint.send(peer.address, packet)

    def sign_block(self, peer, public_key=None, transaction=None, linked=None):
        """
        ...
        assert linked is None or linked.link_public_key == self.my_peer.public_key, \
        ...
        block = self.BLOCK_CLASS.create(transaction, self.persistence, self.my_peer.public_key,
    ...
    def received_half_block(self, source_address, data):
        ...
        blk = TrustChainBlock([payload.public_key, payload.sequence_number, payload.link_public_key,
                               payload.link_sequence_number, payload.previous_hash, payload.signature,
                               payload.transaction], self.serializer)
        ...
        if blk.link_sequence_number != UNKNOWN_SEQ or \
                blk.link_public_key != self.my_peer.public_key or \
    ...
    def received_crawl_request(self, source_address, data):
        ...
        self.logger.info("Received crawl request from node %s for sequence number %d",
                         peer.public_key.encode("hex")[-8:],
                         payload.requested_sequence_number)
        sq = payload.requested_sequence_number
        if sq < 0:
            last_block = self.persistence.get_latest(self.my_peer.public_key)
        ...
        blocks = self.persistence.crawl(self.my_peer.public_key, sq)
        count = len(blocks)
